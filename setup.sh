...
function waGetSourceCode() {
    ...
    if [[ ! -d "$SOURCE_PATH" ]]; then
        $SUDO git clone --recurse-submodules --remote-submodules https://github.com/TheWatermelon24/winapps-manjaro.git "$SOURCE_PATH"
    else
...
function waGetInquirer() {
    ...
    if [ ! -d "$SYS_SOURCE_PATH" ] && [ ! -d "$USER_SOURCE_PATH" ]; then
        ...
        curl -o "$INQUIRER" "https://raw.githubusercontent.com/TheWatermelon24/winapps-manjaro/main/install/inquirer.sh"
    fi
